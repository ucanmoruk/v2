...
namespace mKYS.Numune
{
    public partial class RaporMaster : Form
    {
        ...
        public void listele()
        {
			...
			m.Firma_Adi as 'Yükleyen'
            FROM  dbo.Rapor AS r 
            LEFT OUTER JOIN  dbo.Firma AS f ON r.FirmaID = f.ID 
            LEFT OUTER JOIN  dbo.Firma AS k ON r.ProjeID = k.ID
			LEFT OUTER JOIN dbo.Firma As m on r.YukleyenID = m.ID
