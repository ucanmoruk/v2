...
namespace mKYS.Duyuru
{
    public partial class DuyuruYeni : Form
    {
        ...
        public void listele()
        {
            ...
            SqlDataAdapter da = new SqlDataAdapter("select ID, Ad + ' ' + Soyad as 'Personel'  from RootKullanici where Durum = N'Aktif' and ID <> '"+Anasayfa.kullanici+"' ", bgl.baglanti());
            da.Fill(dt);
        ...
        void ekleme()
        {
            try
            {
                ...
                SqlCommand add = new SqlCommand("insert into RootMesaj (PersonelID, Tarih, Konu, Duyuru, Durum) values (@a1, @a2, @a3, @a4, @a5) SET @ID = SCOPE_IDENTITY() ", bgl.baglanti());
                ...
                {
                    SqlCommand komut2 = new SqlCommand("Select ID from RootKullanici where Durum = N'Aktif' and ID <> '" + Anasayfa.kullanici + "'", bgl.baglanti());
                    SqlDataReader dr2 = komut2.ExecuteReader();
                    while (dr2.Read())
                    {
                        pID = Convert.ToInt32(dr2["ID"]);
                        SqlCommand ad = new SqlCommand("insert into RootMesajDurum (PersonelID, DuyuruID, Durum)  values (@a1, @a2, @a3)", bgl.baglanti());
