...
namespace mKYS.Duyuru
{
    public partial class DuyuruListe : Form
    {
        ...
        public void listele()
        {
            ...
            SqlDataAdapter da2 = new SqlDataAdapter("select d.ID, k.ID as 'PersonelID', k.Ad + ' ' + k.Soyad as 'Mesaj Gönderen', d.Tarih, d.Konu, d.Duyuru as 'Mesaj' from RootMesaj d " +
                "inner join RootKullanici k on d.PersonelID = k.ID inner join RootMesajDurum m on d.ID = m.DuyuruID where d.Durum = 'Aktif' and m.Durum = 'Beklemede' and m.PersonelID = '" + Anasayfa.kullanici+"' order by d.Tarih desc", bgl.baglanti());
            da2.Fill(dt2);
            ...
            try
            {
                ...
                {                                   
                    ...
                    for (int i = 0; i < gridView1.SelectedRowsCount; i++)
                    {
                        ...
                        {
                            SqlCommand ad = new SqlCommand("update RootMesaj set Durum = 'Pasif' where ID = '" + dID + "'", bgl.baglanti());
                            ad.ExecuteNonQuery();
                            bgl.baglanti().Close();
            ...
            else
            {
                ...
                if (cikis == DialogResult.Yes)
                {
                    for (int i = 0; i < gridView1.SelectedRowsCount; i++)
                    {
                        ...
                        SqlCommand ad = new SqlCommand("update RootMesajDurum set Durum = N'Okundu' , Tarih = @a1 where DuyuruID = '" + kID + "' and PersonelID = '" + Anasayfa.kullanici + "'", bgl.baglanti());
