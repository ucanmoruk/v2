...
namespace mKYS.Duyuru
{
    public partial class DuyuruEx : Form
    {
        ...
        public void listele()
        {
            ...
            SqlDataAdapter da2 = new SqlDataAdapter(@"select d.DuyuruID as 'ID', d.PersonelID, k.Ad + ' ' + k.Soyad as 'Mesaj Gönderen', d.Tarih as 'Okunma Tarihi', m.Konu, m.Duyuru as 'Mesaj' from RootMesajDurum d 
            inner join RootMesaj m on d.DuyuruID = m.ID 
            inner join RootKullanici k on m.PersonelID = k.ID 
            where d.Durum = 'Okundu' and d.PersonelID =  '" + Anasayfa.kullanici + "' order by d.Tarih desc", bgl.baglanti());
