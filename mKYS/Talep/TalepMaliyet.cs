...
namespace mKYS.Talep
{
    public partial class TalepMaliyet : Form
    {
        ...
        void listele()
        {
            SqlCommand komut2 = new SqlCommand("select ID, TalepNo from StokTalepListe where Aktif = 'Aktif' order by TalepNo", bgl.baglanti());
        ...
        void flistele()
        {
            ...
            SqlDataAdapter da = new SqlDataAdapter("Select ID, Ad from StokTedarikci where Durum= 'Aktif' order by Ad ", bgl.baglanti());
            da.Fill(dt);
        ...
        void detaybul()
        {
            ...
            SqlCommand komutID = new SqlCommand("Select * From StokTalepDetay where ID = '" + tID + "'", bgl.baglanti());
            ...
            {
                SqlCommand komutI = new SqlCommand("Select * From StokMaliyet where tID = '" + tID + "'", bgl.baglanti());
        ...
        void kontrol()
        {
            SqlCommand komutID = new SqlCommand("Select count(ID) From StokMaliyet where tID = '" + tID + "'", bgl.baglanti());
            SqlDataReader drI = komutID.ExecuteReader();
            while (drI.Read())
            {
        ...
        void ekleme()
        {
           cevir();
            ...
            SqlCommand add = new SqlCommand(" insert into StokMaliyet (tID, TedarikciID, Tarih, Tutar, Tl, Dolar, Euro,Durum,Miktar,Birim) values" +
        ...
        void guncelle()
        {
            ...
            SqlCommand add = new SqlCommand(" update StokMaliyet set  TedarikciID=@a2, Tarih=@a3, Tutar=@a4, Tl=@a5, Dolar=@a6, Euro=@a7,Miktar=@a8,Birim=@a9 where tID = '"+tID+"'", bgl.baglanti());
            add.Parameters.AddWithValue("@a2", firmaID);
            add.Parameters.AddWithValue("@a3", dateEdit1.EditValue);
            add.Parameters.AddWithValue("@a4", tutar);
            add.Parameters.AddWithValue("@a5", tl);
        ...
        private void combo_no_EditValueChanged(object sender, EventArgs e)
        {
            ...
            SqlDataAdapter da5 = new SqlDataAdapter(@"SELECT d.ID, l.Kod + ' - ' +l.Ad as 'Sipariş'  from StokTalepDetay d 
            inner join StokListesi l on d.StokKod = l.Kod where d.Durumu = 'Aktif' and d.TalepNo = '"+combo_no.Text+"' order by d.TalepNo", bgl.baglanti());
            da5.Fill(dt5);
        ...
        private void gridLookUpEdit3_EditValueChanged(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                SqlCommand komutID = new SqlCommand("Select * From StokTalepDetay where ID = '" + gridLookUpEdit3.EditValue + "'", bgl.baglanti());
                SqlDataReader drI = komutID.ExecuteReader();
                while (drI.Read())
                {
                    txt_stok.Text = drI["Miktar"].ToString();
                    ...
                    SqlCommand komutaID = new SqlCommand("Select count(ID) From StokMaliyet where tID = '" + gridLookUpEdit3.EditValue + "'", bgl.baglanti());
                    ...
                    {
                        SqlCommand komutI = new SqlCommand("Select * From StokMaliyet where tID = '" + gridLookUpEdit3.EditValue + "'", bgl.baglanti());
                        SqlDataReader dr = komutI.ExecuteReader();
                        while (dr.Read())
                        {
                            gridLookUpEdit2.EditValue = dr["TedarikciID"].ToString();
