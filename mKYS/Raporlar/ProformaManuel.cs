...
{
    public partial class ProformaManuel : DevExpress.XtraReports.UI.XtraReport
    {
        public ProformaManuel()
        {
            InitializeComponent();
        }

        Musteri.ProformaUpdate n = (Musteri.ProformaUpdate)Application.OpenForms["ProformaUpdate"];
        ...
        public void bilgi2()
        {
            ...
            SqlCommand komut = new SqlCommand("select Ad from Kullanici where ID in (select OnaylayanID from ProformaDurum where ProformaNo = '" + pEvrakNo.Value + "')", bgl.baglanti());
            SqlDataReader dr = komut.ExecuteReader();
            while (dr.Read())
            {
                pl = dr["Ad"].ToString();
