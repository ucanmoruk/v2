...
namespace mKYS.Stok
{
    public partial class HammaddeUGDMix : Form
    {
        public HammaddeUGDMix()
        {
            InitializeComponent();
        }
        ...
        void listele()
        {
            ...
            SqlDataAdapter da6 = new SqlDataAdapter("select ID, InciAd, CasNo from rHammadde where Durum = N'Aktif' order by InciAd", bgl.baglanti());
            da6.Fill(dt6);
            gridControl1.DataSource = dt6;
            gridView1.Columns["ID"].Visible = false;

            gridView1.Columns[1].Width = 110;
        ...
        {
            DataTable dt6 = new DataTable();
            SqlDataAdapter da6 = new SqlDataAdapter("select e.ID, e.InciAd, e.CasNo, m.Yuzde from rHammadde e left join mrMix m on e.ID = m.EtkenID where m.MixID ='" + hID + "'  order by InciAd", bgl.baglanti());
            da6.Fill(dt6);
        ...
        {
            DataTable dt6 = new DataTable();
            SqlDataAdapter da6 = new SqlDataAdapter(@"select ID, InciAd, CasNo from rHammadde where Durum = N'Aktif' 
            except select ID, InciAd, CasNo from rHammadde
            where ID in (select EtkenID from mrMix where MixID = '" + hID + "') order by InciAd", bgl.baglanti());
            da6.Fill(dt6);
            gridControl1.DataSource = dt6;
            gridView1.Columns["ID"].Visible = false;
        ...
        void kontrol()
        {
            ...
            SqlCommand komut32 = new SqlCommand("select InciAd from rHammadde where ID = '" + hID + "'  ", bgl.baglanti());
            SqlDataReader dr32 = komut32.ExecuteReader();
            while (dr32.Read())
            {
                hadi = dr32["InciAd"].ToString();
            }
            bgl.baglanti().Close();
        }
        ...
        private void barButtonItem1_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            for (int i = 0; i < gridView1.SelectedRowsCount; i++)
            {
                ...
                SqlCommand add2 = new SqlCommand("BEGIN TRANSACTION " +
                  "insert into mrMix (MixID, EtkenID, Durum, Gelis) " +
                  "values (@o1,@o2, @o3, @o4);" +
                ...
                add2.Parameters.AddWithValue("@o4", "UGD");
