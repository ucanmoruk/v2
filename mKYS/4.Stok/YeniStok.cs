...
namespace mKYS
{
    public partial class YeniStok : Form
    {
        ...
        {
            SqlCommand komut2 = new SqlCommand("Select Count(ID) from RootStokListesi where Kod = N'" + txtkod.Text + "' ", bgl.baglanti());
        ...
        private void ekleme()
        {
            try
            {
                ...
                SqlCommand add = new SqlCommand("insert into RootStokListesi (Tur,Kod,Ad,AdEn,Cas,Ambalaj,Ozellik,Saklama,Limit,Birim,Miktar,Durum,AdInci) values (@a1,@a2,@a3,@a4,@a5,@a6,@a7,@a8,@a9,@a10,@a12,@a11,@a13)", bgl.baglanti());
                ...
                add.Parameters.AddWithValue("@a13", txt_inci.Text);
                add.ExecuteNonQuery();
                bgl.baglanti().Close();
